...
__attribute__ ((naked)) void assert_failed(char const *aModule, int const aLoc);
...
#else
static constexpr std::array ISRVector {
    static_cast<ISRHander>(&__stack_end__),          // Top of Stack
...
void Reset_Handler() {
    ...
    for (unsigned dst{&__data_start}; dst < &__data_end__; ++dst, ++src) {
        *dst = *src;
    }

    // Zero fill the .bss segment in RAM...
    for (unsigned dst{&__bss_start__}; dst < &__bss_end__; ++dst) {
        *dst = 0;
    }
    ...
    if (&software_init_hook != (void (*)())(0)) {
        ...
        [[maybe_unused]] auto lResult = main();
        __libc_fini_array();
    }
